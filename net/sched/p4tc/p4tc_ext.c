...
static const struct p4tc_extern_param_ops ext_param_ops[P4TC_T_MAX + 1] = {
	[P4TC_T_DEV] = {
	...
	if (e->params) {
		...
		idr_for_each_entry(&e->params->params_idr, parm, id) {
			...
			if (val_ops && val_ops->dump_value) {
				if (val_ops->dump_value(skb, parm->ops,
							parm) < 0) {
		      ...
		      bool init_param, struct netlink_ext_ack *extack)
{
	...
	*attrs_size +=
		nla_total_size(BITS_TO_BYTES(param->type->container_bitsz));
	nparam->id = param->id;
...
#define p4tc_ext_init_params(net, control_params_idr, params, nla, atrrs_size, \
			     extack) \
	(__p4tc_ext_init_params(net, control_params_idr, params, \
				nla, &(attrs_size), true, extack))

#define p4tc_ext_parse_params(net, control_params_idr, params, nla, \
			      attrs_size, extack) \
	(__p4tc_ext_init_params(net, control_params_idr, params, \
			    ...
			    struct p4tc_extern_common *e)
{
	...
	if (refcount_dec_and_test(&e->p4tc_ext_refcnt)) {
		idr_for_each_entry_ul(&e->params->params_idr, param, tmp,
				      param_id) {
...
static void p4tc_ext_idr_insert_many(struct p4tc_extern *externs[])
{
	...
	for (i = 0; i < P4TC_MSGBATCH_SIZE; i++) {
		...
		if (inst->tbl_bindable)
			list_add_tail(&e->common.node, &inst->unused_elems);
		spin_unlock_bh(&inst->available_list_lock);
	}
}
			...
			size_t attr_size, struct netlink_ext_ack *extack)
{
	...
	skb = alloc_skb(attr_size <= NLMSG_GOODSIZE ?
			NLMSG_GOODSIZE : attr_size, GFP_KERNEL);
	if (!skb)
		return -ENOBUFS;
