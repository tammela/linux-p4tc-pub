...
p4a_runt_prealloc_init_param(struct idr *params_idr,
			     struct p4tc_act_param *param,
...
static void p4a_runt_prealloc_params_list_free(struct list_head *params_list)
{
...
static void p4a_runt_prealloc_list_free(struct p4tc_act *act,
					struct list_head *acts_list)
{
	struct tcf_p4act *p4act, *p;

	list_for_each_entry_safe(p4act, p, acts_list, node) {
		struct tc_action *tcf_act = &p4act->common;

		list_del(&p4act->node);
		tcf_idr_release(tcf_act, false);
	}
}

static int p4a_runt_prealloc_params_init(struct idr *params_idr,
					 struct list_head *params_lst,
					 struct netlink_ext_ack *extack)
{
	struct p4tc_act_param *param;
	...
	idr_for_each_entry_ul(params_idr, param, tmp, param_id) {
		...
		nparam = p4a_runt_prealloc_init_param(params_idr, param,
						      &param_id, extack);
		if (IS_ERR(nparam))
...
p4a_runt_prealloc_list_add(struct p4tc_act *act_tmpl,
			   struct list_head *acts_lst)
{
	struct tcf_p4act *p4act, *p;

	list_for_each_entry_safe(p4act, p, acts_lst, node) {
		list_del_init(&p4act->node);
		list_add_tail(&p4act->node, &act_tmpl->prealloc_list);
		tcf_idr_insert_1(&p4act->common);
	}
}

static int
p4a_runt_prealloc_create(struct net *net, struct p4tc_act *act,
			 struct idr *params_idr, struct list_head *act_lst,
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	for (i = 0; i < num_prealloc_acts; i++) {
		...
		err = p4a_runt_prealloc_params_init(params_idr, &params_lst,
						    extack);
		if (err < 0) {
			p4a_runt_prealloc_params_list_free(&params_lst);
			goto destroy_acts;
		}

		err = p4a_runt_init_from_tmpl(net, (struct tc_action **)&a, act,
					      params_idr, &params_lst, &parm,
					      flags, extack);
		p4a_runt_prealloc_params_list_free(&params_lst);
		if (err < 0)
			goto destroy_acts;

		list_add_tail(&a->node, act_lst);
	}

	return 0;

destroy_acts:
	p4a_runt_prealloc_list_free(act, act_lst);
		...
		u32 flags, struct netlink_ext_ack *extack)
{
	...
	if (active == 1) {
		...
		p4a_runt_prealloc_list_add(act, &acts_lst);
