...
{
	struct p4tc_table_entry *first_matched_entry = NULL;
	...
	rhl_for_each_entry_rcu(entry, tmp, bucket_list, ht_node) {
		...
		/* We'll only get here with !prio if table type is LPM */
		if (!prio)
			first_matched_entry = entry;
		else if (value->prio == prio)
			return entry;
		i++;
	}
	...
	if (!prio && i) {
		if (i == 1)
			return first_matched_entry;
		/* In this case the user didn't specify prio, but the bucket had
		 * more than one entry and needed disambiguation
		 */
...
__must_hold(RCU)
{
	...
	if (from_control) {
		...
		ret = p4tc_tbl_entry_proc_name_build(entry_work->who_deleted,
						     params);
		if (ret < 0)
			pr_warn("Unable to copy kernel proc name for deletion");

		entry_work->who_deleted_ent = P4TC_ENTITY_KERNEL;
		entry_work->who_deleted_pid = 0;
	}
		       ...
		       struct p4tc_table_entry_create_bpf_params *params)
{
	...
	/* We only arrive here if the table type is exact, so only possible
	 * return values here are a valid entry or NULL
	 */
		    ...
		    struct netlink_ext_ack *extack)
{
	...
	if (table->tbl_type == P4TC_TABLE_TYPE_EXACT) {
		...
		if (table->tbl_type == P4TC_TABLE_TYPE_TERNARY) {
			if (NL_REQ_ATTR_CHECK(extack, arg, tb,
					      P4TC_ENTRY_PRIO)) {
				NL_SET_ERR_MSG(extack,
	...
	if (!entry) {
		...
		NL_SET_ERR_MSG(extack,
			       "Must specify entry priority to disambiguate");
		ret = PTR_ERR(entry);
		goto unlock;
	}
...
__must_hold(RCU)
{
	...
	if (!entry_old) {
		ret = -ENOENT;
		goto rm_masks_idr;
	} else if (IS_ERR(entry_old)) {
		ret = PTR_ERR(entry_old);
		goto rm_masks_idr;
	}
		      ...
		      struct netlink_ext_ack *extack)
{
	...
	if (tb[P4TC_ENTRY_ACT]) {
		...
		if (!ret) {
			NL_SET_ERR_MSG(extack, "Entry action attr is empty");
			ret = -EINVAL;
			goto free_entry;
		}
